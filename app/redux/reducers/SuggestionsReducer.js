    ...
    switch (action.type) {
        case UPDATE_EXERCISE_SUGGESTIONS:
            return Object.assign({}, state, {
                exerciseModel: generateAutocompleteExerciseModel(
                    action.workoutData,
                    action.historyData,
                ),
            });
        case UPDATE_TAG_SUGGESTIONS:
            return Object.assign({}, state, {
                tagsModel: generateAutocompleteTagsModel(
                    action.workoutData,
                    action.historyData,
                ),
            });
...
const generateAutocompleteExerciseModel = (workoutData, historyData) => {
    ...
    sets.forEach(set => {
        if (set.exercise) {
            ...
            if (dictionary[lowercaseExercise] === undefined) {
                dictionary[lowercaseExercise] = {
                    suggestion: lowercaseExercise,
                    seed: 1,
                };
            } else {
                dictionary[lowercaseExercise] = {
                    suggestion: lowercaseExercise,
                    seed: dictionary[lowercaseExercise].seed + 1,
                };
            }
        }
    });
    ...
    for (var property in dictionary) {
        if (
            dictionary.hasOwnProperty(property) &&
            dictionary[property].seed > 1
        ) {
...
const generateAutocompleteTagsModel = (workoutData, historyData) => {
    ...
    sets = sets.filter(
        set =>
            set.tags !== undefined && set.tags !== null && set.tags.length > 0,
    );
    ...
    sets.map(set => {
        set.tags.map(tag => {
            ...
            if (model[lowercaseTag] === undefined) {
                ...
                model[lowercaseTag] = {
                    suggestion: lowercaseTag,
                    seed: model[lowercaseTag].seed + 1,
                };
            }
        });
    });
