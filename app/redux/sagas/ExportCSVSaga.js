...
import { EXPORTING_CSV } from 'app/configs+constants/ActionTypes';

export default function* FetchConfigSaga() {
    yield all([takeEvery(EXPORTING_CSV, exportCSV)]);
}
...
function* exportCSV(action) {
    ...
    try {
        ...
        try {
            ...
            console.tron.log(
                `Export CSV failed to unlink filepath on err ${err}`,
            );
        }
    }
}
