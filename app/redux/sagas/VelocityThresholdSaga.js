...
import { ADD_REP_DATA } from 'app/configs+constants/ActionTypes';
...
function* playVelocityThresholdAudio(action) {
    const velocityThreshold = yield select(
        SettingsSelectors.getVelocityThreshold,
    );
    const currentRepVelocity = action.averageVelocity / 1000;

    if (currentRepVelocity < velocityThreshold) {
        console.tron.log(
            currentRepVelocity + ' less than ' + velocityThreshold,
        );
