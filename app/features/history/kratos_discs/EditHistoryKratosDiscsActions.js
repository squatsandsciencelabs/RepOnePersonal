...
export const saveKratosDiscs = (setID, kratosDiscs = []) => (dispatch, getState) => {
    const state = getState();
...
const logSaveKratosDiscsAnalytics = (state) => {
    const duration = DurationsSelectors.getEditHistoryKratosDiscsDuration(state);

    Analytics.logEventWithAppState('save_kratos_discs', {
        value: duration,
        duration: duration,
...
const logCancelEditKratosDiscsAnalytics = (state) => {
    ...
    Analytics.logEventWithAppState('cancel_edit_kratos_discs', {
        value: duration,
        duration: duration,
        is_working_set: false,
    }, state);
...
const logRemovedKratosDiscsAnalytics = (state) => {
    Analytics.logEventWithAppState('remove_kratos_disc', {
        is_working_set: false,
    }, state);
};

const logAddKratosDiscAnalytics = (state) => {
    Analytics.logEventWithAppState('add_kratos_disc', {
        is_working_set: false,
    }, state);
};
