import { DISMISS_SELECT_EXERCISE } from 'app/configs+constants/ActionTypes';
...
export const saveSelected1RMExercise =
    (exercise = 'Squat') =>
    (dispatch, getState) => {
        const state = getState();
...
const logCancelEditExerciseAnalytics = state => {
    Analytics.logEventWithAppState('one_rm_cancel_edit_exercise', {}, state);
};

const logSaveExerciseAnalytics = (state, exercise) => {
    Analytics.logEventWithAppState(
        'one_rm_select_exercise',
        {
