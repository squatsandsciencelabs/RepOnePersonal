...
export const exportCSV = () => async (dispatch, getState) => {
    ...
    if (state.auth.email === null) {
        Alert.alert(
            'Sign in Required',
            'Please sign in via Settings to use this feature.',
        );
        return;
    }
        ...
        if (result === null) {
            ...
            try {
                ...
                await GoogleDriveUploader.upload(
                    ...
                    fileID => {
                        ...
                        Linking.openURL(
                            ...
                            Alert.alert(
                                'Upload Succeeded',
                                'CSV uploaded to your Google Drive!',
                            );
                        });
                ...
                if (
                    ...
                    // TODO: should do typeof check but it's not working
                    Alert.alert(
                        'Google Drive Permissions Error',
                        'Please log out and log in again. This feature requires additional Google Drive permissions.',
                    );
                } else {
                    Alert.alert(
                        'Error exporting CSV',
...
const updateIsExportingCSV = isExportingCSV => ({
    type: EXPORTING_CSV,
...
const logTapGoogleSignInAnalytics = state => {
    Analytics.logEventWithAppState('tap_google_sign_in', {}, state);
};
...
const logAttemptCSVAnalytics = state => {
    ...
    const time_since_last_workout =
        SetsSelectors.getTimeSinceLastWorkout(state);
...
const logAttemptLogoutAnalytics = state => {
    Analytics.logEventWithAppState('attempt_logout', {}, state);
};

const logLogoutCancelledAnalytics = state => {
    Analytics.logEventWithAppState('logout_cancelled', {}, state);
};
